...
function fillAuthTree() {
	...
	if (ajaxReturn.status != 200) {
		layer.msg("请求处理出错响应状态码 是 ： " + ajaxReturn.status + " 说 明 是 ："
...
function getPageInfoRemote() {
	...
	// 如果当前请求失败，状态码不是200
	if (statusCode !== 200) {
		layer.msg("服务器端程序调用失败，响应状态码：" + statusCode + "说明信息："
...
function fillTableBody(pageInfo) {
	...
	for (var i = 0; i < pageInfo.list.length; i++) {
		...
		var checkBtn = "<button type='button' id='"
				+ roleId
				+ "' class='btn btn-success btn-xs checkBtn'><i class=' glyphicon glyphicon-check'></i></button>"
		// 通过button标签的id属性，吧roleId的值传递给单击响应函数
		var pencilBtn = "<button type='button' id='"
				+ roleId
				+ "' class='btn btn-primary btn-xs pencilBtn'><i class=' glyphicon glyphicon-pencil'></i></button>"
		var dangerBtn = "<button type='button' id='"
				+ roleId
				+ "' class='btn btn-danger btn-xs deleteBtn'><i class=' glyphicon glyphicon-remove'></i></button>"

		var buttonTd = "<td>" + checkBtn + " " + pencilBtn + " " + dangerBtn
				+ "</td>"
